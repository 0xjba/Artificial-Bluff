...
- Deck seed: 16 random bytes per live game (never derived from the public game id). A running game's config is
  withheld (`/api/games/:id` config null) and published once it is over. Its events so far are readable (they hold no
  seeds; a running study's duplicate deck seeds are masked), so live viewers can seek back ("time shift": a seek bar
  and hand steps; the past plays on at replay pace until it catches up; the LIVE tag dims meanwhile and jumps back).
