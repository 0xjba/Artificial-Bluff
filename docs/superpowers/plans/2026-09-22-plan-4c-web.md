...
- **Feed:** `/api/feed` goes through the Next rewrite to the live server (checked: SSE streams, not buffered). Only the public read API is rewritten (feed, state, health, games), never `/api/admin/*`; rewrites are compiled at build time, so set `API_URL` for `next build`. A snapshot resets the client state; messages for another channel are ignored. The browser retries dropped connections itself but not after an error status, so `useFeed` reopens with backoff (3 s → 30 s) and reports connecting/open/lost. The lower third's "true" chance is the decider's equity just before they acted (`decisionEquity`). Browser replays have no true equity (the server computes it); they apply each event exactly once whatever the pause/speed clicks. Fallbacks are named in plain words ("connection lost: seat auto-played", "timed out", …; spec §9). (Final-review findings.)
...
- **Sounds:** synthesised with Web Audio (no files), muted by default, remembered in `localStorage` (guarded); unmuting resumes the audio context from the click; one sound per new log line (by line identity, so a restart or new programme still sounds; joining is silent).
- **Security:** `/research/[id]/[file]` serves only allow-listed file names of plain folder names (links use the folder: mock reports are `<id>-mock`), regular files only, the HTML report with a strict sandboxed CSP; malformed or old `report.json` files are skipped. Replay ids are checked and fetches time out after 10 s; `app/error.tsx` catches render failures.
- **Generated files:** `.next/`, `next-env.d.ts`, `*.tsbuildinfo` and the `AGENTS.md`/`CLAUDE.md` that `next dev` writes into `apps/web` are git-ignored; typecheck works without them.
...
next-env.d.ts
*.tsbuildinfo
# written by `next dev` on every run (pointers to the bundled Next.js docs)
apps/web/AGENTS.md
apps/web/CLAUDE.md
```
...
- Modify: `apps/web/package.json` (dev dependency `jsdom`)
- Test: `apps/web/test/reports.test.ts`, `apps/web/test/client.test.tsx`

- [ ] **Step 1: Write the failing tests**

Run `pnpm --filter @ab/web add -D jsdom@26` (the client tests run in jsdom).
...
`apps/web/test/client.test.tsx` (jsdom: replays never apply an event twice, sounds are gated, the feed reconnects):
```ts
...
Run: `pnpm --filter @ab/web exec vitest run test/reports.test.ts test/client.test.tsx` → FAIL (cannot resolve `../lib/reports`, `../components/ReplayScreen`).
...
`apps/web/app/error.tsx` (in Next 16 the recovery prop is `retry`):
```ts
...
Run: `pnpm test && pnpm typecheck` → PASS (engine 112, mascot 91, players 44, core 39, analysis 20, server 42, study 49, web 15); clean.
