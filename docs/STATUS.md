...
- 2026-09-22: Plan 5 Tasks 3-5 implemented on `feat/plan-5-byo-table` (Tasks 1-2 already committed): Task 3 keys.ts (visitor key storage, tab vs device; Sign in with OpenRouter OAuth PKCE) at 99da4d9; Task 4 relay.ts + `/api/typesafe/[...path]` route (stateless TypeSafe relay for Jev seats: path/origin/auth/body checks, per-address rate limit, no cookies or extra headers forwarded) at dde7636; Task 5 table.ts (`checkSetup`, `buildPlayers`, `LocalTable` — plays a turbo tournament in the browser, models straight to OpenRouter and Jev through the relay, emits the same feed messages as the live server, stops after the hand in progress, ends at the spending cap) at 1fd9ecf. All written verbatim from the plan; no deviations. 41 web tests (was 33), 441 tests total across the workspace; `pnpm test && pnpm typecheck` clean at every commit. No real OpenRouter/TypeSafe calls made (fakes only). Next: remaining Plan 5 tasks (PlaySetup/PlayScreen, /play page) per the plan.
