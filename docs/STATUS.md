...
- 2026-09-24: user approved the $25 main study. Before the first hand, logging was extended so the paper has everything (bcef3d3): each decision records TypeSafe's own pick (`jevChoice`, before `chooseMove`), the OpenRouter host (`provider`), and a model's raw reply when unusable (`rawReply`, 1000 chars per attempt); decision exports add reasoning, fallbackReason, jevChoice, provider, rawReply and `at`; reports add hands.csv (per hand and player, with group/rotation/seed) and events.jsonl (whole log); the Research page links them (e9a1d11); the paper reports how often the rule changed Jev's move and a "Served by" column in Table 2 (0b99af0). Pre-registration committed at studies/prereg/main-2026-09.json, hash 167fc6d580c6d2773cd2e7afa36c01f52893c738d538a80d59b6e4456cd6d0fa (unchanged by the logging changes). Run started (`pnpm study run studies/main.example.json --live`, log in the session scratchpad): first 11 hands in ~3 min, $0.059/hand, no fallbacks; Claude Fable served by Anthropic and Google; expected ~425 hands, ~2 h, ending budget_cap. After it ends: `pnpm study report studies/main.example.json` (free), check the paper, publish to reports/ and the VPS.
- 2026-09-24: card animations (user: "very mechanical"): `DealtCard` pitches each card in face down and flips it over (the back is a CSS ::after behind the face, so it is a real 3D flip); hole cards go out one seat at a time from the left of the button, two rounds, and the flop turns left to right (`lib/deal.ts`: holeDelay, boardDelay, wonHand; tested). A fold shows `MuckedCards` (two backs, tossed crooked, dimmed) instead of dashes; the pot's winner gets a gold seat glow and its cards lift; board cards lift on hover; seat highlights ease; `prefers-reduced-motion` turns it all off. Checked on desktop and phone (sizes, seek bar, no sideways scroll). Also fixed web test fixtures for the paper's new `hosts`/`moveRule` facts.
- 2026-09-24: MAIN STUDY DONE: main-2026-09 ended budget_cap, 600 hands (120 groups, 30 blocks), 4,741 decisions, $25.01, zero fallbacks for every model. Report + paper in reports/main-2026-09 (git-ignored; copy to the VPS per docs/deploy.md). Results: Jev median 885 ms (2.3x faster than Gemini 2.04 s; Fable 4.81 s), $0.000039/decision (3.7x cheaper than Llama $0.00014; Fable $0.0157). Pre-registered headline calibration (A, vs main-pot share actually won): Jev WORST, Brier 0.247 vs 0.107-0.174 (Astra and Gemini best, 0.107). Against the true chance (C): Jev best ECE 0.011 (others 0.039-0.133) but less sharp: off truth 18.0 pts (Gemini 14.0, Fable 15.4), SD of stated chances 8.8 pts vs 15.9-22.9 (true 25.3). Calls right 47% (worst, with Llama 48%); folds 79%. Style VPIP 63%/PFR 48% (others 19-42%/14-25%); the move rule changed 338 of 1,076 Jev moves (31%), all towards raising. Chips: Jev -70 bb/100 [-380, 240]; no contrast significant (Holm p = 1.0). Paper reworked so it reports all of this: headline outcome A in its own table (Table 4) and in the abstract, ECE vs spread explained, accuracy and style sentences, results-based discussion and conclusion, hosts as a line under each Table 2 row, ties on the headline named together. Research page tiles now include the headline rank and ECE (lean and fallback tiles dropped).
