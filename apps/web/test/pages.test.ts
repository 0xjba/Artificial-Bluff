...
describe('research figures', () => {
  const table: ModelsTable = {
    ...
    hands: 80,
    seats: [
      seat({ playerId: 'jev', kind: 'jev', honestyGapPts: -2, costPerDecisionUsd: 0.0009, latencyMeanMs: 240, decisions: 100, fallbacks: 0 }),
      seat({ playerId: 'pill', honestyGapPts: 31, costPerDecisionUsd: 0.214, latencyMeanMs: 1620, decisions: 100, fallbacks: 3 }),
    ],
  }
  ...
  it('says only what the numbers say', () => {
    ...
    const split = { ...table, seats: [table.seats[0]!, seat({ playerId: 'pill', honestyGapPts: 1, costPerDecisionUsd: 0.05 })] }
  ...
  it('turns the table into figures, and drops ratios that are not really differences', () => {
    ...
    expect(m[0]).toEqual({ value: '2 pts', what: "average gap between JEV's stated win chance and the true one" })
    expect(m[1]!.value).toBe('31 pts')
    expect(m[2]!.value).toBe('238×') // $0.214 against $0.0009
    expect(m[3]!.value).toBe('6.8×') // 1.62 s against 240 ms
    ...
    const alike = { ...table, seats: [table.seats[0]!, seat({ playerId: 'pill', honestyGapPts: 9, costPerDecisionUsd: 0.001, latencyMeanMs: 250 })] }
